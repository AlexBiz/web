...
$to = "zvonok@yuzhpharm.ru"; //получатель уведомлений
...
if ($interval < 1) { // интервал отправки (сек)
	...
	if (count($get_data) > 1) { // data to send
		...
		$title 	= "Заказ звонка с сайта YUZHPHARM.RU";
		...
		$headers	.= "From: Администратор сайта yuzhpharm.ru <".$from.">\r\n"; 

		$sms['msg'] = translit((getOptions(0)));
		$sms['msg'] = substr($sms['msg'], 0, 160);

		if ($to != 'support@yuzhpharm.ru') { 
